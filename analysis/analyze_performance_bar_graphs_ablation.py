...
def Plot(
	...
	if target.__contains__("IoU"):
		...
		plt.figure(figsize=(1.25*len(presets), 3))
	...
	colors = ["#87CEEB", "#4682B4", "#4169E1", "#FF961E"]
	labels = ["Prior_Metadata", "Prior_LLF", "Prior_VIFF", "Prior_LLF_VIFF"]
	for i in range(4):
		if i<3:
			continue
		...
		bars = plt.bar(
			...
			color=[colors[i]], label="W/O Compression Statistics"
		)
	...
	for i in range(4):
		...
		bars = plt.bar(
			...
			height=Y, width=f*width, 
			color=[colors[i]], label="With Compression Statistics"
		)
		add_text_anotations(bars, target)
	...
	if target.__contains__("IoU") or target.__contains__("f_"):
		if codec == "libx265" or codec == "libsvtav1":
			plt.legend(markerscale=1.1, loc='upper center', bbox_to_anchor=(0.5, 1.125), ncol=4, fancybox=True, shadow=True, fontsize=7)
		else:
			plt.legend(markerscale=1.1, loc='upper center', bbox_to_anchor=(0.5, 1.25), ncol=2, fancybox=True, shadow=True, fontsize=7)
	else:
		if codec == "libx265" or codec == "libsvtav1":
			plt.legend(markerscale=1.1, loc='upper center', bbox_to_anchor=(0.5, 1.125), ncol=1, fancybox=True, shadow=True, fontsize=7)
		else:
			plt.legend(markerscale=1.1, loc='upper center', bbox_to_anchor=(0.5, 1.125), ncol=1, fancybox=True, shadow=True, fontsize=7)
	
	# Set X and Y Ticks
	if target.__contains__("IoU") or target.__contains__("f_"):
		plt.xticks(ticks=X-2.5*width, labels=presets, fontsize=7)
	else:
		plt.xticks(ticks=X-2.5*width, labels=presets, fontsize=7)
...
if __name__ == "__main__":
	...
	for codec in ["libx265", "libsvtav1", "libaom-av1", "libvpx-vp9"]:
		Plot(
			...
			save_dir="plots/performance/bar_graphs/ablation/libx265"
		)
	...
	for codec in ["libx265", "libsvtav1", "libaom-av1", "libvpx-vp9"]:
		Plot(
			...
			save_dir="plots/performance/bar_graphs/ablation/libsvtav1"
		)